...
Base_Worker.prototype.loadConfiguration = function() {
	if (process.env.TP_CB_HOST && process.env.TP_CB_BUCKET) {
	...
	if (process.env.TP_KFK_HOST && process.env.TP_KFK_PORT && process.env.TP_KFK_CLIENT) {
		this.config.kafka = {
	...
	if (process.env.TP_REDIS_HOST && process.env.TP_REDIS_PORT) {
		this.config.redis = {
