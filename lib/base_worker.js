...
var Base_Worker = function(type, index) {
	...
	this.broadcastChannel = 'channel-transport';
...
Base_Worker.writeDelta = function(delta, callback) {
	...
	transaction.zcard('telepat:deltas');
	transaction.zadd(['telepat:deltas', delta.ts, JSON.stringify(delta.toObject())]);
	transaction.exec(function(err, replies) {
		if (err)
			return callback(new Error('Transaction failed: writing delta ('+err.toString()+')'));
 ...
 */
Base_Worker.popDeltas = function(count, callback) {
	var transaction = Models.Application.redisClient.multi();

	transaction.zrange(['telepat:deltas', 0, count]);
	transaction.zremrangebyrank(['telepat:deltas', 0, count]);
	transaction.zcard('telepat:deltas');
	transaction.exec(function(err, replies) {
		if (err)
			return callback(new Error('Transaction failed: getting and/or removing deltas ('+err.toString()+')'));

		var results = [];

		if (Array.isArray(replies[0])) {
			replies[0].forEach(function(d) {
				results.push(JSON.parse(d));
			});
		}

		callback(null, results, replies[2]);
	});
};
 ...
 * @param callback
 */
Base_Worker.multiGetAndRemoveDeltas = function(channels, callback) {
		...
		transaction.exec(function(err, replies) {
			...
			replies.slice(0, deltaKeys.length).forEach(function(deltasArray, index) {
				...
				deltasArray.forEach(function(patchValue, patchIndex, patchesArray) {
					...
					if (['replace', 'increment', 'append', 'remove'].indexOf(patchesArray[patchIndex].op) !== -1) {
						var pathParts = patchesArray[patchIndex].path.split('/'); //modelname/id/fieldname
						var appId = patchesArray[patchIndex].subscription.split(':')[1];
...
Base_Worker.prototype.notifyWriter = function(redisKey, callback) {
	this.messagingClient.send([JSON.stringify({key: redisKey})], 'write', callback);
};

Base_Worker.getDeviceIdsFromSubscription = function(channel, callback) {
	Models.Application.redisClient.smembers(channel, callback);
};

Base_Worker.getDevices = function(deviceIds, callback) {
	if (!deviceIds.length)
