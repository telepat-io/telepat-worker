...
UpdateFriendsWorker.prototype = Object.create(Base_Worker.prototype);
		...
		async.waterfall([
			...
			function(userResults, callback) {
				async.each(userResults.hits.hits, function(user, callback1) {
					Models.User.update(user._source.id, user._source.application_id,
						[{op: 'append', path: 'user/'+user._source.id+'/friends', value: message.fid}], callback1);
				}, callback);
			}
		], function(err, result){
			if (err) {
