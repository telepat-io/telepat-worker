...
function Aggregate(message) {
    switch(message.op) {
        case 'add': {
            ...
            var mdl = item.type.charAt(0).toUpperCase() + item.type.slice(1);
            ...
            for (var r in Models.Model._spec[mdl].belongsTo) {
                if (item[Models.Model._spec[mdl].belongsTo[r].parentModel+'_id'])
                    parent = {model: mdl, id: item[Models.Model._spec[mdl].belongsTo[r]+'_id']};
            }
        ...
        case 'edit': {
            ...
            var id = message.id;
            var mdl = message.type.charAt(0).toUpperCase() + message.type.slice(1);

            async.waterfall([
                function(callback) {
                    new Models.Model(mdl, id, context, callback);
                },
                function(result, callback) {
                    var userId = result.value.user_id;
                    var parent = null;

                    for (var r in Models.Model._spec[mdl].belongsTo) {
                        if (result.value[Models.Model._spec[mdl].belongsTo[r].parentModel+'_id'])
                            parent = {model: mdl, id: result.value[Models.Model._spec[mdl].belongsTo[r]+'_id']};
                    }
        ...
        case 'delete': {
            ...
            var context = message.object.context;
            var mdl = message.object.type.charAt(0).toUpperCase() + message.object.type.slice(1);
                            ...
                            if (key) {
                                stateBucket.getAndLock(key, function(err, results){
                                    if (err && err.code == cb.errors.keyNotFound) {
                                        ...
                                        var items = results.value;
                                        items.deltas = item.deltas.concat(patch);
