			...
			case 'add': {
				...
				async.waterfall([
					...
					function writeDeltas(result, callback) {
						 ...
						 */
						if (result === false) {
							stateBucket.insert(subKey, {deltas: [{op: 'add', value: userProps, type: 'user', guid: guid, ts: process.hrtime().join('') }]}, callback);
						} else {
							var items = result.value;
							items.deltas.push({op: 'add', value: userProps, guid: guid, type: 'user', ts: process.hrtime().join('')});
