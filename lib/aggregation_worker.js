...
AggregationWorker.prototype.processMessage = function(message) {
	...
	async.series([
		function createDelta(callback) {
							...
							if (err) {
								...
								self.validateAuthor(result, 'write_acl', delta.patch.user_id, function(err1) {
									if (!err1) {
										...
										Models.Application.logger.error(err1.message);
									}
									c();
								});
							}
			...
			} else if (message.op == 'update_schema') {
					Models.Application.updateSchema(message.object.id, message.object.schema, function(err, result) {
						if (err) {
							Models.Application.logger.error(err.message);
						} else {
							Models.Application.loadedAppModels[message.object.id].schema = message.object.schema;
							console.log("delta to write = ", delta);
							deltasToWrite.push(delta);
		...
		function notifyWriters(callback) {
		    ...
		    console.log("deltas" , deltasInDb ,deltasToWrite.length);
...
AggregationWorker.prototype.validateAuthor = function(object, aclType, userId, callback) {
	...
	var builtinModels = ['application', 'admin', 'user', 'user_metadata', 'context'];
	if ((appSchema &&  !appSchema[object.type]) || builtinModels.indexOf(object.type))
		return callback();
	if ((appSchema[object.type][aclType] & 8) && userId) {
		...
		return callback(new Models.TelepatError(Models.TelepatError.errors.OperationNotAllowed));
	} else {
		callback();
	}
};
