	...
	async.waterfall([
			...
			if (operationtype == Base_Worker.OP.UPDATE || operationtype == Base_Worker.OP.DELETE) {
				if (message.isUser) {
					if (message.user) {
						...
						Models.User({username: message.object.username}, appId, function(err, result) {
							if (err) {
								var errMsgOperation = 'OP_'+operationtype.toUpperCase();
								err.message = errMsgOperation+': Retrieving user with username \''+message.object.username+'\': '+
								err.message;
				...
				} else if (message.isContext) {
					// message.context exists when context is deleted instantly
					if (message.context) {
						object = message.context;
						callback();
					} else {
						var conextId = message.object.path.split('/')[1];
						Models.Context(conextId, function(err, result) {
							if (err) {
								var errMsgOperation = 'OP_'+operationtype.toUpperCase();
								err.message = errMsgOperation+': Retrieving context with id \''+conextId+'\': '+
									err.message;
					...
					Models.Model(pathParts[0], message.applicationId, message.context, pathParts[1], function(err, result) {
						if (err) {
							var errMsgOperation = 'OP_'+operationtype.toUpperCase();
							err.message = errMsgOperation+': Retrieving model: '+err.message;
			...
			if (operationtype == Base_Worker.OP.UPDATE) {
				...
				self.filterLatePatches(patches, updateTimestamp, appId, function(err, filteredPatches) {
					...
					filteredPatches.forEach(function(patch) {
						...
						if (message.isUser)
							createdDelta.username = message.object.username;

						if (message.instant)
							createdDelta.instant = true;

						createdDelta.application_id = appId;

						//only application models have context_id
				...
				if (operationtype == Base_Worker.OP.ADD)
					...
					if (message.isUser)	{
						createdDelta.username = message.object.username;
						createdDelta.user = message.user;
					}
				}
			...
			async.each(subscriptionChannels, function(channel, c) {
				async.each(deltaObjects, function(mainDelta, c1) {
					...
					Base_Worker.writeDelta(d, function(err, deltasLength) {
						if (err) {
							err.message = 'Error writing deltas: '+err.message;
		...
		function notifyWorkers(channelsWithNewDeltas, callback) {
			...
			if (channelsWithNewDeltas.length === 0)	{
				Models.Application.logger.info('No subscription keys with new deltas. Writers not notified.');
				return callback();
			}

			self.notifyWriter(channelsWithNewDeltas, message.applicationId, function(err) {
				if (err) {
					err.message = 'Failed notifying workers: '+err.message;
				}
				callback(err);
			});
		}
	], function(err) {
		if (err) Models.Application.logger.error(err.toString());
	});
};
...
AggregationWorker.prototype.filterLatePatches = function(patches, updateTimestamp, appId, callback) {
	...
	async.filter(patches, function(itemPatch, filterCallback) {
		...
		if (!results.length) {
			Models.Application.logger.notice('All patches has been discarded due to being outdated.');
			return callback(null, []);
		}
...
AggregationWorker.prototype.notifyWriter = function(subscriptionKeys, appId, callback) {
	...
	async.map(subscriptionKeys, function(channel, c) {
		...
		self.messagingClient.send([JSON.stringify({keys: results, applicationId: appId})], 'write', callback);
	});
};
