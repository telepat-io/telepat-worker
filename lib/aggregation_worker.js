	...
	async.series([
		...
		function notifyWriters(callback) {
			if (deltasInDb == 0 && deltasToWrite.length) {
				tlib.services.messagingClient.send([JSON.stringify({ key: [self.name + ':deltas'] })], 'write', callback);
			}
			else
				callback();
		}
...
AggregationWorker.prototype.getAffectedChannels = function (item, appId, callback) {
	...
	var modelsBaseChannel = (new tlib.channel(appId)).model(item.type, item.id);
...
AggregationWorker.prototype.validateAuthor = function (object, aclType, userId, callback) {
	...
	if (tlib.apps[appId].modelSchema(object.type).isValidModel() !== true) {
		return callback();
	}
	if ((tlib.apps[appId].modelSchema(object.type).aclType & 8) && userId) {
		...
		return callback(new tlib.TelepatError(tlib.TelepatError.errors.OperationNotAllowed));
	} else {
		callback();
	}
};
