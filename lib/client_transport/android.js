		...
		if (err) {
			...
			if (failures) {
				async.forEach(failures, function(failedDevice, c2) {
					async.series([
						function(callback) {
							Models.Subscription.removeAllSubscriptionsFromDevice(applicationId, failedDevice.id, failedDevice.token, 'android_transport', callback);
						},
						function(callback) {
				...
				async.forEach(canonical, function(canonicalDevice, c2) {
					...
					async.series([
						function(callback) {
							Models.Subscription.removeAllSubscriptionsFromDevice(applicationId, canonicalDevice.id, canonicalDevice.token, 'android_transport', function(err, results) {
								if (err) {
									return callback(err);
								}
