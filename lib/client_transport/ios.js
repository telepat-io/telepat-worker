...
iOSTransport.prototype.sendNotification = function(application_id, apnNotification, iosDevice) {
	...
	Models.Application(application_id, function(err, app) {
		...
		if (!apnConnections[application_id]) {
			...
			apnFeedbacks[application_id].on('feedback', self.feedbackFunction);
		} else {
			var hasChanged = null;
			...
			hasChanged = apnConnections[application_id].options.passphrase != app.apn_passphrase;
...
iOSTransport.prototype.processMessage = function(message) {
	...
	async.filter(message.deltas.updated, function(patch, filterCallback) {
		...
		if (message.device.persistent && message.device.persistent.token) {
			...
			collectedDeviceIds[message.device.persistent.token] = {id: message.device.id, app: message.device.application_id};

			var apnNotification = new apn.Notification();
			apnNotification.contentAvailable = 1;
			apnNotification.sound = '';

			if (JSON.stringify(message.deltas).length > 1836) {
				...
				async.forEachOfSeries(message.deltas, function(deltas, key, c) {
					deltas.forEach(function(d) {
						if (key == 'new') {
							...
							if (iosPushProperty && d.value[iosPushProperty]) {
				...
				message.deltas.new = message.deltas.new.filter(function(delta) {
				...
				if (message.deltas.new.length || message.deltas.updated.length || message.deltas.deleted.length) {
					apnNotification.payload = {data: message.deltas, deviceID: message.device.id};
					console.log(JSON.stringify(apnNotification.payload).length);
					self.sendNotification(message.device.application_id, apnNotification, iosDevice);
				}
			}
		} else {
