	...
	async.series([
		...
		async.forEachOfSeries(broadcast ? payload : message.payload, function(deltas, pnToken, c) {
			...
			if (JSON.stringify(deltas.deltas).length > 1920) {
				...
				deltas.deltas.new = deltas.deltas.new.map(function(delta) {
					var mdl = delta.object.type;
					var iosPushProperty = mdl ? Models.Application.loadedAppModels[delta.object.application_id].schema[mdl]
					&& Models.Application.loadedAppModels[delta.object.application_id].schema[mdl].ios_push_field : null;
					...
					if (iosPushProperty && delta.value[iosPushProperty]) {
						...
						self.sendNotification(delta.object.application_id, specialNotification, iosDevice);
					} else
						return delta;
				});

				apnNotification.payload = {data: deltas.deltas};
