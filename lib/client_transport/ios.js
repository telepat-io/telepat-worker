...
iOSTransport.prototype.sendNotification = function(applicaion_id, apnNotification, iosDevice) {
	if (!apnConnections[applicaion_id]) {
		var app = Models.Application.loadedAppModels[applicaion_id];

		if (!app)
			return Models.Application.logger.error('Error sending notification: application '+applicaion_id+' not found');

		if (!app.apn_key || !app.apn_cert || !app.apn_passphrase)
			return Models.Application.logger.error('Application '+applicaion_id+' is not configured for APN');

		apnConnections[applicaion_id] = new apn.Connection({key: app.apn_key, cert: app.apn_cert, passphrase: app.apn_passphrase, production: true});
	}

	apnConnections[applicaion_id].pushNotification(apnNotification, iosDevice);
};
