...
iOSTransport.prototype.feedbackFunction = function(devices) {
	var collectedDeviceIdsReference = collectedDeviceIds;
	collectedDeviceIds = {};

	async.each(devices, function(d, c) {
		var device = collectedDeviceIdsReference[d.device];

		if (device) {
			async.series([
				function(callback) {
					Models.Subscription.removeAllSubscriptionsFromDevice(device.app, device.id, callback);
				},
				function(callback) {
					Models.Subscription.removeDevice(device.app, device.id, callback);
				}
			]);
		}
	...
	Models.Application(application_id, function(err, app) {
		...
		if (!apnConnections[application_id]) {
			if (!app.apn_pfx){
				...
				apnFeedbacks[application_id] = new apn.Feedback({
					batchFeedback: true,
					interval: 1000,
					key: app.apn_key,
					cert: app.apn_cert,
					passphrase: app.apn_passphrase,
					production: true
				});
			} else	{
				...
				apnFeedbacks[application_id] = new apn.Feedback({
					batchFeedback: true,
					interval: 1000,
					pfx: new Buffer(app.apn_pfx, 'base64'),
					passphrase: app.apn_passphrase,
					production: true
				});
			}
