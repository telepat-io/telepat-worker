...
iOSTransport.prototype.removeInvalidDevices = function(tokens) {
	async.each(tokens, function(token, c) {
		...
		if (dev) {
			async.series([
				function(callback) {
					Models.Subscription.removeAllSubscriptionsFromDevice(dev.app, dev.id, token, 'ios_transport', callback);
				},
				function(callback) {
				...
				if (err) {
					return Models.Application.logger.error('Error removing device and/or subscriptions: ' + err.message);
				}

				Models.Application.logger.debug('Device ' + JSON.stringify({id: dev.id, token: token}) + ' removed due to permanent failure from APN');
			});
		}
...
iOSTransport.prototype.sendNotification = function(applicationId, apnNotification, iosTokens) {
	...
	if ((!app.apn_key && !app.apn_key_id && !app.apn_team_id && !app.apn_topic)) {
	...
	apnConnections[applicationId].send(apnNotification, iosTokens).then(function (response) {
		if (response.failed.length) {
			...
			response.failed.forEach(function(device) {
				...
				if (device.response && device.response.reason === 'BadDeviceToken' && collectedDeviceIds[token]) {
					...
					Models.Application.logger.warning('Failed sending notification to device '
		...
		if (response.sent.length) {
			Models.Application.logger.debug('Sent ' + response.sent.length + ' notifications');
		}
	});
};
