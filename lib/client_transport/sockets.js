...
var SocketClientTransport = function(index) {
	...
	this.socketIo.on('connection', function(socket) {
		...
		socket.on('bindDevice', function(data) {
			...
			Models.Subscription.updateDevice(data.application_id, data.deviceId, {server_name: self.name},
				function(err) {
					if (err) Models.Application.logger.error('Error setting server name to device: '+err.message);

					Models.Subscription.activateDevice(data.application_id, data.deviceId, function(err1) {
						if (err1) Models.Application.logger.error('activateDevice: '+err.message);
					});
				});
		});
