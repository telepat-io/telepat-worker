...
var SocketClientTransport = function(index) {
	...
	this.socketIo.on('connection', function(socket) {
		...
		socket.on('bindDevice', function(data) {
			...
			Models.Subscription.activateDevice(data.application_id, data.device.id, function(err) {
				if (err) {
					Models.Application.logger.error('activateDevice: '+err.message);
				}
			});
		});
		...
		socket.on('disconnect', function() {
			...
			if (self.sockets[socket.id].device) {
				...
				Models.Subscription.deactivateDevice(appId, device.id, function(err) {
					if (err) {
