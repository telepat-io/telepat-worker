...
var SocketClientTransport = function(index) {
	...
	this.socketIo.on('connection', function(socket) {
		...
		socket.on('bindDevice', function(data) {
			...
			Models.Subscription.updateDevice(data.application_id, data.device.id, {connected: true}, function(err) {
				if (err) {
					Models.Application.logger.error('updateDevice: '+err.message);
				}
			});
		});
		...
		socket.on('disconnect', function() {
			...
			if (self.sockets[socket.id].device) {
				...
				Models.Subscription.updateDevice(appId, device.id, {connected: false}, function(err) {
					if (err) {
						Models.Application.logger.error('updateDevice: '+err.message);
					}
