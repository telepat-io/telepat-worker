...
var SocketClientTransport = function(index) {
	...
	var port = process.env.TP_SCKT_PORT || 81;
	...
	var portSSL = process.env.TP_SCKT_PORT_SSL || 443;
	...
	if (privateKeyFile && certificateFile) {
		...
		this.socketIoSSL = require('socket.io')(httpsServer);
	}

	this.socketIoSSL.on('connection', onConnectListener);
