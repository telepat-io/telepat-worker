					...
					function removeSubscriptions(callback) {
						Models.Subscription.getDevice(appId, deviceId, function(err, device) {
							...
							if (device.subscriptions.length) {
								...
								device.subscriptions.forEach(function(sub) {
									transaction.srem([sub, self.name+'|'+deviceId+'|'+socket.id+'|'+appId]);
								});

								transaction.exec(function(err) {
									if (err) {
										return callback(new Error('Failed removing subscription keys: '+err.message));
									}
									callback();
								});
							} else {
...
SocketClientTransport.prototype.processMessage = function(message) {
	...
	async.forEach(payload, function(groupedSubscription, c) {
			...
			async.each(groupedSubscription.deltas, function(d, c2) {
				...
				async.each(groupedSubscription.deviceTokens, function(deviceIdentifier, c2) {
					...
					if (self.sockets[token]) {
						...
						Models.Application.logger.error('Unable to send notification to socket with ID ' + token + ' : '
