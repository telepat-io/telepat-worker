					...
					function(callback1) {
						async.each(Object.keys(subscribedDevices), function (d, callback2) {
							if (subscribedDevices[d].transport_type == 'email') {
								...
								if (subscribedDevices[d].volatile.active) {
									if (deviceTokens[subscribedDevices[d].volatile.type] === undefined)
										deviceTokens[subscribedDevices[d].volatile.type] = [{token: subscribedDevices[d].volatile.token, id: deviceTokens[subscribedDevices[d].id]}];
									else
										deviceTokens[subscribedDevices[d].volatile.type].push({token: subscribedDevices[d].volatile.token, id: deviceTokens[subscribedDevices[d].id]});

								} else {
									if (deviceTokens[subscribedDevices[d].persistent.type] === undefined)
										deviceTokens[subscribedDevices[d].persistent.type] = [{token: subscribedDevices[d].persistent.token, id: deviceTokens[subscribedDevices[d].id]}];
									else
										deviceTokens[subscribedDevices[d].persistent.type].push({token: subscribedDevices[d].persistent.token, id: deviceTokens[subscribedDevices[d].id]});
								}
							 }
							callback2();
						}, function(err) {
							for(var transportType in Object.keys(deviceTokens)) {
								var topicName = transportType+'_transport';

								kafkaProducer.send([{
									topic: topicName,
									messages: [JSON.stringify({devices: deviceTokens[transportType], data: clientPatches})]
								}]);
							}
