    ...
    async.waterfall([
        ...
        function getDeltas(callback) {
            stateBucket.getMulti(keys, function(err, results) {
				...
				if (err == keys.length)
					return callback(new Error('No deltas available in the database'.red+'. '+err+' keys requested'));

				async.each(Object.keys(results), function(subKey, c) {
					if (results[subKey].error) {
						console.log(('Deltas key '+subKey+' was not found.').red);
						return c();
					}
					var subKeyParts = subKey.split(':');
					subKeyParts.pop();
					var realSubKey = subKeyParts.join(':');
					results[subKey].value.deltas.forEach(function(value, key, array) {
						array[key].subscription = realSubKey;
					});
                    ...
                    case "increment" : {
						async.detectSeries(modifiedItemsPatches, function(modP, detectCallback) {
							...
							if(res && (res.subscription == d.subscription)) {
								...
								modifiedItemsPatches.push({op: d.op, path: d.path, value: d.value, subscription: d.subscription, guid: d.guid});
