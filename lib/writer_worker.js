...
WriterWorker.prototype.getAndRemoveDeltas = function(redisKey, callback) {
	...
	transaction.exec(function(err, replies) {
		...
		async.each(replies[0], function(r, c) {
			...
			callback(null, deltas, replies[1]);
		});
	});
};
	...
	async.each(deltas, function(delta, c) {
		switch(delta.op) {
			...
			case 'update':
				...
				break;
			case 'delete':
				deleteObjects.push(delta.object);
		...
		if (createDeltas.length) {
			Models.Model.create(createDeltas, function(err) {
				...
				callback(null, createdItems.concat(updatePatches, deleteObjects));
			});
		}
...
WriterWorker.prototype.createItems = function(deltas, callback) {
	...
	async.eachSeries(deltas, function(d, c) {
		it++;

		if (self.operationIds[deltas[it-1].guid])
			return c();

		if (deltas[it-1].value.type == 'user') {
			deltas[it-1].value.application_id = deltas[it - 1].application_id;

			self.operationIds[deltas[it-1].guid] = true;
