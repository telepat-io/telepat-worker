	...
	async.whilst(function() { return it < deltas.length; },
		function(callback1) {
			...
			if (deltas[it-1].value.type == 'user') {
				...
				Models.Model.create(deltas[it - 1].value.type, deltas[it - 1].context, deltas[it - 1].application_id, deltas[it - 1].value, function (err1, result1) {
		...
		async.detectSeries(deletedItemsDeltas, function(delItem, c) {
			...
			if (result)	{
				...
				if (pathParts[0] == 'user') {
					...
					Models.Model.update(pathParts[0], item.context, item.application_id, pathParts[1], [modItem], callback1);
				}
			}
		});
	}, callback);
