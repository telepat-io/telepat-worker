...
WriterWorker.prototype.updateItems = function(deltas, deletedItemsDeltas, callback) {
	...
	async.each(deltas, function(item, callback1) {
		...
		async.detectSeries(deletedItemsDeltas, function(delItem, c) {
			...
			if (result)	{
				...
				if (pathParts[0] == 'user') {
					Models.User.update(item.email, [item], callback1);
				} else if (pathParts[0] == 'context') {
					Models.Context.update(pathParts[1], [item], callback1);
				} else {
					var modItem = cloneObject(item);
					modItem.path = pathParts[2];
					Models.Model.update(pathParts[0], item.application_id, pathParts[1], [modItem], callback1);
				}
			}
		});
	}, callback);
...
WriterWorker.prototype.deleteItems = function(deltas, callback) {
	...
	async.each(deltas, function(delItem, callback1){
		...
		if (pathParts[0] == 'user') {
			...
			Models.Model.delete(pathParts[0], delItem.application_id, pathParts[1], false, callback1);
		}
	}, callback);
};
...
WriterWorker.prototype.sendClientNotifications = function(deltas, subscribedDevices, appId, callback) {
	...
	async.each(subscribedDevices, function(device, c) {
		...
		var transportMessage = {device: null, deltas: [], applicationId: appId};
		...
		async.each(device.subscriptions, function(subscription, c1) {
			if (deltas[subscription])
				transportMessage.deltas.push(deltas[subscription]);
			c1();
		}, function() {
...
WriterWorker.prototype.deleteUserRelatedKeys = function(user, callback) {
	async.waterfall([
