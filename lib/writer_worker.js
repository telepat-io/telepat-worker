	...
	async.series([
		...
		function(callback1) {
			async.each(Object.keys(objectPatches), function(objectPath, c) {
				...
				if (pathParts[0] == 'user') {
					Models.User.update(objectPatches[objectPath][0].email, objectPatches[objectPath], callback1);
				} else if (pathParts[0] == 'context') {
					Models.Context.update(pathParts[1], objectPatches[objectPath], callback1);
				} else {
					Models.Model.update(pathParts[0], objectPatches[objectPath][0].context, objectPatches[objectPath][0].application_id, pathParts[1], objectPatches[objectPath], callback1);
				}
