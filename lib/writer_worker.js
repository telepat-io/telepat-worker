...
WriterWorker.prototype.processMessage = function(message) {
	...
	async.series([
		...
		if (!remainingDeltas && self.fetchCount > 1)
			self.fetchCount /= 2;
		else if (remainingDeltas && self.fetchCount < 512) {
			self.messagingClient.send([JSON.stringify({key: message.key})], 'write', function(err) {
				if (err) {
					...
					Models.Application.logger.debug('Sent additional message to writers to grab deltas (fetchCount at '+self.fetchCount*2+')');
				}
			});
			self.fetchCount *= 2;
		}
