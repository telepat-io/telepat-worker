		...
		async.each(device.subscriptions, function(subscription, c1) {
			...
			if (transportType == 'sockets') {
				if (!device.server_name) {
					...
					self.messagingClient.send([JSON.stringify(transportMessage)], device.server_name, function(err) {
						if (err) {
							Models.Application.logger.error('Failed to send message to '+device.server_name+': '+err.message);
						}
					});
				}
			} else {
