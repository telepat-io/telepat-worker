...
WriterWorker.prototype.processMessage = function (message) {
	...
	console.log('de ce pula mea nu se contecteaza la redis');
	var self = this;
	var deltas = [];
	...
	async.series([
		...
		function processDeltas(callback) {
			console.log("process deltas", deltas);
...
WriterWorker.prototype.getAndRemoveDeltas = function (redisKey, callback) {
	...
	console.log('here, get and remove detas');
	...
	async.each(deltas, function (delta, c) {
		switch (delta.op) {
			case 'create':
				...
				console.log("CREATE DELTAS", delta);
				break;
			case 'update':
				if (!delta.instant)
		...
		console.log('create deltas', createDeltas);
		if (createDeltas.length) {
			let createModels=[];

			for (let i = 0; i < createDeltas.length; i++) {
				if (createDeltas[i].object.type === 'user') {
					console.log("calling create users on ", createDeltas[i].object);
		...
		if (updatePatches.length) {
			...
			console.log("UPDATE DELTAS", updatePatches);
			...
			if (updateSeparator.model.length) {
				console.log('begun to update models in write worker ', updateSeparator.model);
			...
			if (updateSeparator.user.length) {
				console.log ('begun to update users', updateSeparator.user);
