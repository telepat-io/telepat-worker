...
TransportManager.prototype.processMessage = function(message) {
	...
	async.series([
		...
		function getDevices(callback) {
			...
			async.forEachOf(transportMessages, function(devices, transportName, c) {
				self.messagingClient.send([JSON.stringify({payload: devices})], transportName, function(err) {
					if (err)
						Models.Application.logger.error('Failed sending message to '+transportName+': '+err.message);
				});
				c();
			}, callback);
		}
