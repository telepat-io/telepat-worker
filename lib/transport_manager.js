		...
		async.series([
			...
			function getDevices(callback) {
				...
				async.each(Object.keys(collectedDevices), function(dev, c) {
					...
					transportMessages[keySplit[0]][keySplit[1]] = {deltas: [], application_id: keySplit[2]};//collectedDevices[dev];

					var deltaIds = {};

					async.forEachSeries(collectedSubscriptions, function(deltas, c2) {
						var deltaCopy = [];

						async.forEachSeries(deltas, function(item, c3) {
							...
							if (deltaIds[deltaId] !== true) {
								deltaIds[deltaId] = true;

								var clonedItem = clone(item);
								clonedItem.subscriptions = commonSubs;

								deltaCopy.push(clonedItem);
							}
							c3();
						}, function() {
							if (deltaCopy.length)
								transportMessages[keySplit[0]][keySplit[1]].deltas =
									transportMessages[keySplit[0]][keySplit[1]].deltas.concat(deltaCopy);
