			...
			function getDevices(callback) {
				...
				async.each(Object.keys(collectedDevices), function(dev, c) {
					...
					async.each(collectedDevices[dev], function(sub, c2) {
						if (collectedSubscriptions[sub]) {
							var deltaCopy = collectedSubscriptions[sub].map(function(item) {
								item.subscriptions = _.intersection(item.subscriptions, collectedDevices[dev]);
								return item;
							});
				...
				async.forEachOf(transportMessages, function(devices, deviceString, c) {
					...
					self.messagingClient.send([JSON.stringify({payload: devices, application_id: deviceString.split('|')[2]})], queueName, function(err) {
						if (err)
