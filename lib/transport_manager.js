		...
		async.series([
			...
			function getDevices(callback) {
						...
						async.forEach(deltas, function(item, c3) {
							...
							if (item.subscriptions.indexOf(sub) !== -1 && collectedDevices[dev].indexOf(sub) !== -1) {
								...
								if (item.op == 'create' || item.op == 'delete') {
									objectIdentifier = item.object.id;
									md5sum.update(objectIdentifier);
								} else if (item.op == 'update') {
									objectIdentifier = JSON.stringify(item.patch);
									md5sum.update(objectIdentifier);
								}

								md5sum.update(sub);
								md5sum.update(keySplit[0]);
				...
				async.forEachOf(transportMessages, function(subscriptionDeltas, queueName, c) {
					tlib.services.messagingClient.send([JSON.stringify({payload: subscriptionDeltas})], queueName, function(err) {
						if (err)
							tlib.services.logger.error('Failed sending message to '+queueName+': '+err.message);
					});
					c();
				}, callback);
			}
		...
		async.series([
			function(callback) {
				...
				async.forEach(allQueues, function(q, c) {
					tlib.services.messagingClient.send([JSON.stringify({payload: {'*': {deltas: deltas, applicationId: deltas[0].application_id}}})], q, function(err) {
						if (err) {
							tlib.services.logger.error('Failed sending message to '+q+': '+err.message);
						}
					});
					c();
				});
				...
				async.forEach(broadcastQueues, function(q, c) {
					tlib.services.messagingClient.publish([JSON.stringify({payload: {'*': {deltas: deltas, applicationId: deltas[0].application_id}}})], q, function(err) {
						if (err) {
							tlib.services.logger.error('Failed broadcasting message to '+q+': '+err.message);
						}
					});
					c();
				});
